"""非破壞性匯出薩滿歐莎拉之鏡／怪力魔獎勵 transaction。"""
...
TARGET_FUNCTION_EAS = (0x14312, 0x118B0)
...
for requested_ea in TARGET_FUNCTION_EAS:
    ...
    while ea != idaapi.BADADDR and ea < fn.end_ea:
        row = instruction(ea)
        row["function_original"] = ida_funcs.get_func_name(fn.start_ea)
        rows.append(row)
        ea = ida_bytes.next_head(ea, fn.end_ea)

result = {
    "schema": "dq3.ida_samanosa_mirror_reward_evidence.v1",
    "input": {
        "path": str(input_path),
        "size": len(raw),
