...
# 淡入淡出 concat,末段鋪背景音樂(預設精訊 MT-32 地表曲 track_00.ogg;DQ3_DEMO_BGM 可覆寫;缺則退回原創 chiptune)。
...
CHIPTUNE="$OUT/demo_bgm.wav"
# BGM:預設用精訊 MT-32 地表曲 track_00.ogg(使用者要求);可用 DQ3_DEMO_BGM 覆寫;缺則退回原創 chiptune。
# ⚠ IP:track_00 含 DQ3 著作權旋律(精訊資料經 MT-32 render),產出 mp4 gitignore 不入庫;對外公開請自行斟酌。
BGM="${DQ3_DEMO_BGM:-$ROOT/work/mt32/track_00.ogg}"
...
# ---------- BGM 來源 ----------
if [ -f "$BGM" ]; then
  echo "== BGM:$BGM(精訊 MT-32 地表曲)=="
else
  echo "== BGM 來源缺,退回原創 chiptune =="
  [ -f "$CHIPTUNE" ] || docker run --rm -v "$ROOT":/work -w /work ghcr.io/astral-sh/uv:python3.12-bookworm-slim \
...
docker run --rm -v "$ROOT":/repo -v dq3build:/build dq3-remake bash -lc '
  ...
  # 城鎮日夜 NPC:同一城(阿里阿罕 CTY00)白天 vs 黑夜 → section 兩份 NPC 表(白天人多/黑夜人少回屋)
  mk daytown   && env $E DQ3_RECDIR=$R/daytown   DQ3_DEBUG="party;dn:0;warp:0:4:24" DQ3_INPUT="uuuurrrruuuullllrrrr" $B $A game >/dev/null 2>&1 || true
  mk nighttown && env $E DQ3_RECDIR=$R/nighttown DQ3_DEBUG="party;dn:2;warp:0:4:24" DQ3_INPUT="uuuurrrruuuullllrrrr" $B $A game >/dev/null 2>&1 || true
  ...
  for d in title field night daytown nighttown battle sokoban townpre townpost hydra father zoma; do echo "   $d: $(ls $R/$d/*.ppm 2>/dev/null|wc -l) 幀"; done
'
...
card "$TMP/c02b.mp4" 4.4 "城鎮日夜:NPC 不一樣" "白天人多走動,黑夜多數回屋睡覺、有的隱藏、有的夜間才現 —— 每 section 內建白天/黑夜兩份 NPC 表(RE docs/60)"; add "$TMP/c02b.mp4"
clip "$REC/daytown"   "$TMP/s02b.mp4" 3.8 "阿里阿罕 · 白天(NPC 多,熱鬧)"        && add "$TMP/s02b.mp4"
clip "$REC/nighttown" "$TMP/s02c.mp4" 3.8 "阿里阿罕 · 黑夜(同地點,人回屋變少)"   && add "$TMP/s02c.mp4"
...
echo "   鋪背景配樂 $(basename "$BGM")(${DUR}s,淡入淡出)…"
if [ -f "$BGM" ]; then
