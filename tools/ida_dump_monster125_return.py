"""非破壞性匯出 monster125 action／逃跑與返航遭遇所需靜態證據。

IDA Pro 9.4 batch：
  idat -A '-Stools/ida_dump_monster125_return.py OUT D3MNS.DAT' DQ3.EXE
...
DESCRIPTORS_IDA = 0x28593 # DGROUP 0x37c3, 60 records × 3 bytes
REQUESTED = (0x19834, 0x199DC, 0x19AD6, 0x19B4C, 0x1A1B4, 0x1A51A, 0x1A973, 0x1BCC4)
        ...
        "xrefs_to": [
            {
                ...
                "caller_original": ida_funcs.get_func_name(
...
def window(start, size):
    return {
        "start_ida_linear": hex(start), "end_ida_linear": hex(start + size),
...
def set_bits(mask):
    # sub_19AD6 由每 byte 的 0x80 向右掃，故 raw index 是 MSB-first。
...
def action_for(bit, remap):
    ...
    out = {"mask_bit": bit, "action_raw": action, "mapping_source": source,
           "level": "confirmed raw mapping; semantic name requires consumer review"}
    descriptor = ida_bytes.get_bytes(DESCRIPTORS_IDA + action * 3, 3) or b""
    out["descriptor"] = {
        "dgroup": hex(0x37C3 + action * 3),
        "ida_linear": hex(DESCRIPTORS_IDA + action * 3),
    ...
    if action >= 0x14:
        ...
        out["handler"] = {
            "entry_ida_linear": hex(entry), "raw_word_le": hex(logical),
            "ida_linear": hex(0x10000 + logical), "logical": hex(logical),
...
def main():
    ...
    if len(rec) != STRIDE:
        raise RuntimeError("monster125 record truncated")
    mask = rec[14:20]
    ...
    payload = {
        ...
        "monster_record": {
            "id": MONSTER_ID, "stride": STRIDE, "file_offset": hex(start),
            "raw_hex": rec.hex(),
            "selected_raw_fields": {
                "hp_base_u16": int.from_bytes(rec[0:2], "little"),
                "hp_rand_u16": int.from_bytes(rec[2:4], "little"),
                "mp_u16": int.from_bytes(rec[4:6], "little"),
                "attack_u8": rec[8], "defense_u16": int.from_bytes(rec[9:11], "little"),
                "agility_u16": int.from_bytes(rec[11:13], "little"),
                "action_gate_u8": rec[13], "mask_hex": mask.hex(), "set_bits": bits,
                "flee_threshold_u8": rec[23], "flee_rate_u8": rec[24],
        ...
        "remap": {"dgroup": "0x3930", "ida_linear": hex(REMAP_IDA),
                  "raw_30_bytes_hex": bytes(remap).hex()},
        "functions": {hex(ea): function_record(ea) for ea in REQUESTED},
        "action41_handler_window": window(0x1A1B4, 0x100),
    }
