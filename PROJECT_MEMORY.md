...
- R-3 已由本機原版影片與 EXE 重新定案：祠堂是 CTY70 sec0（不是舊 C/docs 的 CTY82）；
  六祭壇事件 8f–94、視覺旗標 12c–131、守護神 handler69、拉米亞停泊 `(48,189)`，
  走上搭乘、飛行 Confirm 依 `attr&0x64` 降落。
...
- R-2 已完成事件切片與原版行為驗收。
- R-3 六珠來源、祭壇、復活動畫、飛行／降落及存檔已實作；仍須併入完整無 debug 主線
  playthrough 才能提升為從前一節點自然可達的完成狀態。
...
4. R-2 已於 2026-07-28 完成事件切片：EXE `0x5682..0x5732` 證實 gate 為
   flag42 + 夜 + CTY44 sec1 + 玩家 `(14,7)`，沒有 facing；正式輸入從命令／道具 UI
   經 rec97/98 打贏怪89，勝敗 flags、變身杖0x62、白天與 save/load 均已回歸。
5. 存檔現已保存並重建 story bits、日夜、世界層、CTY/section/座標；修正非阿里阿罕讀檔錯載 CTY00。
6. R-3 已完成 EXE/CTY/影片交叉驗證與 Ebiten event E3；完整紀錄見 `docs/75-phoenix-orbs-re.md`。
   下一批進 R-4 巴拉摩斯城與自然下降；R-2/R-3 目前均不得誤稱已由新遊戲無斷點自然抵達。
