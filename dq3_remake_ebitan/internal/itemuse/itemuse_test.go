...
func TestKindOf(t *testing.T) {
	cases := []struct {
		...
		{0x5a, None}, {0x0f, None}, // 蓋亞之劍由 game-pack item_use_effects 提供
		{0x5e, None}, // 版本專屬乾渴壺效果由 game-pack JSON 提供
		{0x77, FairyFlute}, {0x75, Rainbow},
...
func TestConsumable(t *testing.T) {
	...
	for _, k := range []Kind{PrayerRing, FairyFlute, Mirror, None} {
		if k.Consumable() {
