/* dq3_runtime.c — DOS runtime 的 SDL2 實作(adapters at the edges)。
 * 把原版 far-call runtime 的語意換成 SDL2 / stdio。內部複雜度藏在此檔。
 */
...
static uint8_t       g_fb[DQ3_SCREEN_W * DQ3_SCREEN_H];   /* 4bpp packed 索引 */
...
int dq3_rt_init(const char *title)
{
    ...
    /* nearest-neighbor:底圖放大保持銳利(retro-cjk-hires-canvas)。 */
...
void dq3_set_palette(const dq3_color *pal, int count)
{
    ...
    for (i = 0; i < count; i++) {
        g_pal[i] = pal[i];
        g_pal32[i] = 0xFF000000u | (pal[i].r << 16) | (pal[i].g << 8) | pal[i].b;
    }
}

void dq3_present(void)
{
    void *pixels; int pitch; int i, n = DQ3_SCREEN_W * DQ3_SCREEN_H;
...
/* ---- 輸入:SDL keysym -> BIOS scancode(主迴圈用方向鍵 + 選單鍵)---- */
static uint8_t sdl_to_bios_scancode(SDL_Keycode k)
{
    switch (k) {
        case SDLK_DOWN:  return 0x50;   /* SC_DOWN  (docs/08) */
        case SDLK_LEFT:  return 0x4b;   /* SC_LEFT  */
        case SDLK_UP:    return 0x48;   /* SC_UP    */
        case SDLK_RIGHT: return 0x4d;   /* SC_RIGHT */
...
uint8_t dq3_poll_scancode(void)
{
    ...
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) g_quit = 1;
            ...
            if (e.key.keysym.sym == SDLK_ESCAPE) g_quit = 1;  /* host 退出 */
            else { uint8_t s = sdl_to_bios_scancode(e.key.keysym.sym);
                   if (s) sc = s; }
        }
    }
    return sc;
...
void dq3_delay_ms(uint32_t ms) { SDL_Delay(ms); }
uint32_t dq3_ticks_ms(void)    { return SDL_GetTicks(); }
