...
#define DQ3_SAVE_MAGIC "DQ3SAVE6"   /* v6:加存 storyflags(劇情進度)+ save_pos.daynight(晝夜相位)*/
#define MAGIC_LEN 8

/* 各結構大小寫進檔頭,讀檔時比對 → 防跨版本誤讀。 */
static void sizes(uint32_t s[5])
{
    ...
    s[4] = (uint32_t)sizeof(dq3_storyflags);
}
...
{
    uint32_t s[5]; FILE *f = fopen(path, "wb");
    if (!f) return -1;
    sizes(s);
    ...
    fwrite(flags, sizeof *flags, 1, f);     /* v6:劇情旗標 */
    fclose(f);
    return 0;
}
...
{
    char magic[MAGIC_LEN]; uint32_t s[5], want[5];
    ...
    if (fread(r, sizeof *r, 1, f) != 1 || fread(p, sizeof *p, 1, f) != 1 ||
        fread(inv, sizeof *inv, 1, f) != 1 || fread(pos, sizeof *pos, 1, f) != 1 ||
        fread(flags, sizeof *flags, 1, f) != 1) { fclose(f); return -2; }
    fclose(f);
    return 0;
}
