...
int main(void){
    ...
    c.rng_mode=DQ3_RNG_REAL; c.music_enabled=0; c.music_volume=45; c.audio_backend=1; c.combat_info=0;
    CHECK(dq3_config_save(&c,p)==0, "存檔成功");
    { dq3_config c2; dq3_config_default(&c2);
      CHECK(dq3_config_load(&c2,p)==5, "讀回 5 鍵(rng+music+music_vol+audio+combat_info)");
      ...
      CHECK(c2.combat_info==0, "戰鬥資訊開關持久"); }
    printf("== 缺檔不變動 ==\n");
