/* dq3_pcx.c — TIT*.P 標題畫面解碼(ZSoft PCX,640×350,4 plane,RLE)。
 * 對應 tools/title_pcx.py(已驗證)。planar 4bpp → indexed,取檔內 16 色 palette。
 */
                   ...
                   uint8_t *out, int out_w, int out_h, dq3_color pal16[16])
{
    ...
    /* header offset 16:16 色 EGA palette(已是 0..255 範圍) */
    for (i = 0; i < 16; i++) {
    ...
    line = (uint8_t *)malloc((size_t)row_bytes);
    if (!line) return -4;
    memset(out, 0, (size_t)out_w * out_h);
    ip = 128;
    for (row = 0; row < H; row++) {
        int filled = 0;
        while (filled < row_bytes && ip < n) {           /* RLE 解一整列 */
            uint8_t b = d[ip++];
            if ((b & 0xC0) == 0xC0) {
                int cnt = b & 0x3F;
        ...
        /* 4 planes 組回 4bpp:pixel = p0 | p1<<1 | p2<<2 | p3<<3 */
