...
### Polish(非阻塞)
- [ ] **魯拉/蓋美拉 傳送目的地存讀檔持久**(使用者要求,2026-06-28):傳送選單的「去過城鎮」清單目前是
  `run_game` 區域變數 `visited[16]`(cty+最後位置+section),**存讀檔不會保留** → 讀檔後傳送目的地遺失。
  待做:把 visited 清單寫進 `dq3_save`(加版本欄位),讀檔還原;確認存讀後魯拉/翼仍能傳送到先前去過的城鎮。
