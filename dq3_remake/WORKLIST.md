...
- **杜勝利 + 青衫攻略接線（歷史 C prototype 紀錄）**:B 道具鏈(夢幻紅寶石/變身杖/蓋亞之劍/鑰匙鏈/彩虹水滴/領悟之書/王者之劍)、A boss(巴拉摩斯+怨靈122/殭屍123/索瑪124、甘達特金皇冠、甘達特巢穴 CTY14、八頭大蛇 CTY19、**六大魔人怪106**)、不死鳥拉米亞坐騎(六珠)、**蓋亞那跳坑下降閘(flag0x213)**。舊稱「勇氣神殿神父 gate(flag0x13)」已由 Go／Ebitengine 正式切片推翻：`flag0x13` 是 CTY75／47 世界入口 consumer，試煉接受路徑本身不寫入；現行證據見 `docs/100-lancel-courage-blue-orb-production-trace.md`。本列其餘完成度不得代替現行 `docs/74-ebiten-remake-completion-plan.md`。
...
- [x] **戰鬥受傷視覺特效:震動 + 黃/綠閃光 ✅ 完成**(使用者觀察 2026-06-28):原版我方受傷畫面抖動+閃黃/綠光,已實作。
  - ✅ **RE 定位「抖動」機制**(file **0x13846**,補進 docs/13):32 次迴圈 `xor [0x5b07],0x6d60` 切 VGA 顯示起始位址
    (CRTC 0x0C/0x0D via 0x13834)→ 畫面跳;0x1395b latch write-mode 每幀重繪列。閃光非直接寫 DAC port(走 BIOS/記憶體,確切黃綠值未逐一追)。
  - ✅ **實作**(`dq3_battlescene.c`):`play_hurt_fx()` — ① 震動 `shift_fb_x` framebuffer 水平位移、左右跳 12 幀漸小;
    ② 閃光 palette 交替染黃/綠、漸弱(對齊原版兩色 toggle);③ **結束 `dq3_set_palette(g_pal_save)` 還原不殘留**;
    ④ 觸發=主迴圈 do_turn 前後我方 HP 總和下降。
