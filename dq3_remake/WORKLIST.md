...
- [整合(部分)] **城鎮出入口(warp)**:type-2 傳送事件 → 查內建 warp 表 dq3x_warp(0x4ea0,map+座標)→ 踩到門/階梯傳送(game 模式 demo:回地表)。精確 map→場景對映 + 地表城鎮入口→CTY(load_cty 查表)待續 RE。
