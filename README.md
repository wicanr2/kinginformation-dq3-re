...
**核心 / 進度 / 方法論**
- [`docs/74`](docs/74-ebiten-remake-completion-plan.md) — Go/Ebiten remake 現況與唯一完成計畫
...
- 📱 **Android 移植評估 + Go/Ebiten port** — 評估 [`docs/62`](docs/62-golang-ebiten-android-port-eval.md)(Ebiten 行動/觸控/OGG 內建,消掉 SDL2/NDK 三大痛;RE 已完成→翻譯非發現,估 3–6 週)。**已起手** [`dq3_remake_ebitan/`](dq3_remake_ebitan/README.md):階段 1 骨架跑起來,真實 `DQ3.PAL` → 移植 Go parser → Ebiten 渲染(有截圖證明)。SDL2/NDK 路徑見 [`docs/55`](docs/55-android-macos-port-plan.md)
- ⭐ [**經驗教訓 Lessons Learned**](docs/lessons-learned.md) — 多輪協作踩雷 → 根因 → 紀律（別鬼打牆 / 靜態 RE 優先 / 斷言前驗證 / 零回歸改法…）
- [`docs/00`](docs/00-re-methodology.md) 逆向方法論 · [`docs/69`](docs/69-why-remake-looked-done-but-wasnt.md) 失敗經驗與防再犯方法 · [`docs/46`](docs/46-debug-hook.md) 歷史 DEBUG 口
...
**巴拉摩斯本體 + 索瑪原版 formations**（怨靈122 → 殭屍123 → rec72 → 索瑪124；城內大魔人是一般隨機遭遇）/ 不死鳥拉米亞坐騎 /
...
- macOS / Android 移植(規劃見 [`docs/55`](docs/55-android-macos-port-plan.md))。
  Go/Ebiten 細項見 [`docs/74`](docs/74-ebiten-remake-completion-plan.md)。
