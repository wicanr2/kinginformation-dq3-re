...
| **SDL2 現代跨平台重寫**(`dq3_remake/`,C99+SDL2) | ✅ **完整可玩 + 主線可破關 + 73/73 驗證 + 打包**(見下「remake 可玩狀態」)| [`dq3_remake/`](dq3_remake/)、[`dq3_remake/WORKLIST.md`](dq3_remake/WORKLIST.md) |
...
**交付驗證**:`tools/game_tester.sh` **73/73 全綠**（15 單元 + 主線一條龍 + 全 89 城零崩潰 +
boss 事件 + B 道具鏈 + 巴拉摩斯序列 + 甘達特巢穴 + 存讀檔 roundtrip）+ DOSBox oracle（標題逐像素一致、注音 IME 字序一致，
...
確認 ✅、SDL2 跨平台重寫（完整可玩 + 可破關 + 73/73 驗證 + 打包）✅、`[0x722]` runner/region
