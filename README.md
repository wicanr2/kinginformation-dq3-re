...
| **SDL2 現代跨平台重寫**(`dq3_remake/`,C99+SDL2) | ✅ **完整可玩 + 主線可破關 + 90/90 驗證 + 打包**(見下「remake 可玩狀態」)| [`dq3_remake/`](dq3_remake/)、[`dq3_remake/WORKLIST.md`](dq3_remake/WORKLIST.md) |
...
**交付驗證**:`tools/game_tester.sh` **90/90 全綠**（15 單元 + 主線一條龍 + 全 89 城零崩潰 +
...
確認 ✅、SDL2 跨平台重寫（完整可玩 + 可破關 + **90/90 驗證** + **Linux/Windows/AppImage 三包**）✅、
`[0x722]` runner/region scripted-event 系統靜態攻克 ✅、**杜勝利+青衫攻略全流程缺口盤點 10/11 已接**
（含揪出並修正 3 個道具碼標籤錯位 + 1 個賢者轉職 gate correctness bug，見
[`walkthrough-flow-audit.md`](docs/data/walkthrough-flow-audit.md)）✅。剩餘:

- glyph index → Unicode 對照表（OCR 1,476 字模）→ 把劇情 dump 成純 UTF-8。
- `CHINA.FON` 殘留 ~7% 對齊、段內索引表精確語意。
- 攻略缺口長尾（皆**不阻塞破關**）：B-7 耶進貝亞倉庫番 puzzle（乾渴壺已可取，僅 gate）、
  B-6 建城後 NPC 視覺增生（questline 機制已接）、E-11 全破後裝備變洛特系（為精訊版補上「時代結尾」）。
