...
- `sub_1fb36` 以 `[si+2]`／`[si+4]` 作左上角，將 `[si+6]` 加 1 VGA byte、
  `[si+8]` 加 `0x10` pixel，**只備份關閉時要還原的背景區**。
- `sub_1fd30` 才是可見框線 writer：上／下／左／右邊界直接以
  `[si+6]`／`[si+8]` 計算；因此備份區的額外 1 byte／0x10 pixel 不得併入
  玩家可見的 JSON `width`／`height`。
...
可見外框寬=`0x2c*8=352`、高=`0x60=96`，底部為 y=334；
`(360,112)` 是把備份區誤當成畫框的舊結論，已由 `sub_1fd30` 反證並在
`docs/109-battle-hud-rects-re.md` 留下勘誤鏈。文字 renderer 再加 `(16,16)`，
形成 20 個 16px 欄與每頁 4 行。
原版影片同狀態畫面亦顯示
