...
> 2026-07-28 更正:蓋亞下降已由 CTY72→CTY77→`0xfe` 原版轉場鏈接通；
> 舊稱 event86/file0x783d 的段落是漏掉高位址 `0x1` 後誤認的索瑪終戰函式。

## ★ 前提:這是「未發售的早期 build」

精訊版 DQ3 是**未上市的早期版本**。實證觀察:部分劇情對話/事件**內容不全或未接好**——
舊分析曾把索瑪終戰 `logical 0x164cd / file 0x1783d` 誤寫成下降
`logical 0x64cd / file 0x783d`；這項推論已撤銷。真正跳坑資料完整存在於 CTY72 第二筆
transition，目的 CTY77，而 CTY77 的 `0xfe` 出口進 DQ3UND。
