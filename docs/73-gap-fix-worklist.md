...
> 2026-08-22 最後功能順序亦只在 `docs/74` 維護：戰鬥道具 → 實際使用的剩餘怪物 action →
> 剩餘合法野外咒文 → 商店賣出 → 船進城 → 必要選單／玩家可見支線。本歷史 W1–W8 表
> 不得再被當成待辦；PCM wall-clock、DAC／PIT／DMA 平台細節不列入專案 RE。
...
W1-W5 全數完成、獨立核實、commit(52ce817/0269012/5507a0a/f27b51d/13be592)。docs/72 高信心「兩版都缺」缺口(A1-A4)ebitan 全補；歷史 C comparison baseline 的 NPC(D 段)W5 回補。**延後項**:B1 沙曼歐莎怪力魔(需 R-2 RE 座標)、C1 EBG 音效、C2 攻方減半 0xd4f(RE 未定)。A1-A4 的 C 版對等(除 D)未回補,若要 C 對拍需另補。
...
2. R-4/R-5a/R-5b 已完成核心資料接線：王座隱藏樓梯、歐里狄加 rec69/70 與
   e0→0e 旗標交易、戰後回拉達多姆 handler74 冊封。非戰鬥咒文選單及魯拉原版
   20 城表／落點已於 2026-07-28 接線；IDA 已再追通真正 field caster、烈米特
   handler、section MapFlags gate 與四種 MP。其餘工具咒尚待接線，詳見
   `docs/78-field-spell-re.md`。後續補歐里狄加固定戰鬥動畫，並納入
   無 debug 全流程通關稽核。
