...
| D2 | 步42 不死鳥祠堂集六珠復活拉米亞坐騎 | 祭壇 examine,六珠(0x66-0x6b)判定+對話 | grep "phoenix/六珠/不死鳥/拉米亞/orb" **0 命中**(除註解引用外);**整條坐騎機制不存在**——無飛行狀態、無祭壇觸發 | 未查main.c有無等效 | **A**(後段地圖靠飛行才能到,無此步全部卡死) | new-RE/port-C |
