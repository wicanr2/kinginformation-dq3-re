...
- **palette 調暗（歷史近似，已由 2026-08-22 bank selector 取代）**：舊 C／Go 曾依
  四相位縮放 RGB；該數值不是原版設定，不得再作 production parity 證據。
...
- 各相位 palette 的原版 bank 選擇已 `confirmed` 並接入 runtime；RGB 直接來自原始
  `DQ3.PAL`。DAC／現行 PNG 的同地點、同 clock 逐格比色仍是 V3 gate。
