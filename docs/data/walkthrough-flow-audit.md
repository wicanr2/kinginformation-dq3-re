...
> 原 11 項缺口逐一 RE 透 / code 核實後:**4 個是 false gap(早已 data-driven 可取)、1 個是真 bug(已修)、
> 4 個已接線**;剩 3 項(B-7 低價值 + B-6 複雜 + E-11 moot)。每項都 RE 透或 code 核實,無一靠猜。
...
### ⬜ 剩餘(3/11,皆不阻塞破關)
- **B-6 帶商人建城**(Ch33-41):需商人寄存+建城狀態,最大工項。
- **B-7 耶進貝亞倉庫番 puzzle**(Ch26):乾渴壺已可取,僅 gate(低價值)。
- **E-11 post-game 洛特裝備**:遊戲於索瑪結束,基本 moot。

> game_tester 79→**86** 全綠。道具碼權威表見上方「道具碼勘誤」框 + `quest-items.md`。
