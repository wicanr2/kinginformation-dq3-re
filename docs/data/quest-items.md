...
| 0x14 | 草薙之劍 | CTY19 第一戰怪75原始掉落 | ✅ D3MNS `+0x25=0xff/+0x26=0x14`、正式 InputState trace 與 save/load（docs/95）|
...
| 0x57 | 最終鑰匙 | 四島礁 CTY40 寶箱(Ch27);門 tier3 | ✅ schema 0.1.14：CTY40 正式取得；rec421 明確使用才開 tier3 門且不消耗（docs/98、docs/99） |
...
| 0x5e | 乾渴壺 | 耶進貝亞推石密道(Ch26);吸海(Ch27)| ✅ 正式取得＋船上唯一座標使用＋原版 world-state/5×4 patch（docs/97、docs/98）|
...
| 0x68 | 紅寶珠 | 海盜村 CTY27 密道寶箱(Ch32)| ✅ CTY27 `ctrl bit0x40` 可推入口、hidden transition、event0 flag `0x3f`、正式航行／InputState trace 與 save/load（docs/101） |
