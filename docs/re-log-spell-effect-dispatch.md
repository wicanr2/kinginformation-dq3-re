...
## Step 10:✓ 成功產出 — base==0 咒名辨識(font 渲染)

渲染 21 個 base==0 咒名(D3TXT00.FON glyph×32):
| rec | 名 | 原名 | 效果 |
|---|---|---|---|
| 143 | 尼夫拉姆 | ニフラム | 驅逐弱敵 |
| **144** | **拉里荷** | **ラリホー** | **睡眠** ← 玩家不能行動 |
| 145/151/154/155/157/159/160 | 皮歐里姆/拜基魯多/史卡拉/史克魯多/瑪荷甘達/夫巴哈/阿斯特龍 | ピオリム/バイキルト/スカラ/スクルト/マホカンタ/フバーハ/アストロン | **自/友方 buff** |
| 146/147/149/150 | 魯卡尼/魯加南/伯米歐斯/瑪荷多拉 | ルカニ/ルカナン/ボミオス/マホトラ | debuff(守/速/MP)|
| 148 | 巴西魯拉 | バシルーラ | 傳走 |
| **152** | **美達巴尼** | **メダパニ** | **混亂** ← 不能正常行動 |
| 153 | 莫夏斯 | モシャス | 變身 |
| 156 | 瑪荷頓 | マホトーン | 封咒 |
| 158 | 瑪努莎 | マヌーサ | 幻(命中下降)|
| 166/167/168 | 基阿里/基阿里克/薩梅哈 | キアリー/キアリク/ザメハ | 解毒/解麻/解睡 |

⇒ **可用結論**:怪物放 **rec144(睡)/152(混亂)= 玩家不能行動** → 映射 remake **麻痺**。
其餘 base==0 多為 buff/debuff/解狀態(remake 未模型化,skip)。**這是 name-grounded、非臆造**。

## 最終:wiring(怪物施狀態)
do_turn 敵方施咒分支:picked rec ∈ {144,152} → 對目標 `status |= PARALYSIS`(取代傷害)。

---

## ★ 結局:wiring 成功(怪物施狀態閉環)

- bit→rec:rec144(睡)=bit19、rec152(混亂)=bit26(dq3_monster_spell_rec)。
- 會睡眠咒的怪:id 14,38,88,100,107;會混亂:43,72,100,107(D3MNS stride41 spell_mask)。
- do_turn 敵施咒:picked rec∈{144,152} → `party[t].status |= PARALYSIS` + continue。
- **實測**:`DQ3_MON=14 battle` → 「敵0 詠唱拉里荷 → 勇者 陷入睡眠(麻痺)」✓。
  下回合該員不能行動(既有 skip)、戰後帶回 roster、滿月草/教會可解 → **狀態系統全閉環**。

**這趟 10 步反組譯(含 3 步撲空)值得**:證實無效果型別 byte(避免做錯表)→ 改 base 分類 →
font 渲染辨識咒名 → 鎖定 144/152 異常咒。純靜態 RE + data 分析解決了「怪物施狀態」。
