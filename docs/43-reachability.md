...
- ⇒ **gate 1 真正的 blocker = 首次「下降」觸發**(ギアガの大穴掉進アレフガルド)。RE 追到底:
  ...
  - 0xfe 門全在下層 CTY → 首次下降**不經 +0xc 轉場圖**,而是 **scripted 事件用 warp 執行器 0xd1f9
    直接把玩家送進アレフガルド**(註:舊說「子型2 byte4 33-36 = 0xd1f9 warp」是 window-scan **假陽性已撤回**,見 docs/42/44;真正 warp 是 `call 0xd1f9`+固定 struct,docs/44 §5b)。屬
...
(註:「byte4 33-36 = 0xd1f9 warp」為已撤回的假陽性,真正 warp = `call 0xd1f9`+固定 struct,docs/44 §5b)。**這套系統是到「實際可破關」最高槓桿的單一 blocker**
