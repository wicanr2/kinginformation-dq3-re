...
> **遭遇區範圍(機制澄清)**:遭遇戰發生在**所有「非城鎮」場景** —— 不只地表(overworld),
> **洞窟 / 迷宮**走幾步同樣會遇敵;只有**城鎮(安全區)不遇敵**。洞窟/迷宮雖與城鎮同為 CTY
> 場景,但屬遭遇區。判斷依據是 `[0xd77]`(安全/城鎮無遭遇旗標),**不是「在不在地表」**:
> `[0xd77]==0`(非安全區)才推進遭遇計數。
>
> **`[0xd77]` 來源 = CTY section 旗標(RE 解出,file 0x31b2)**:載 CTY section 時
> `ds=[0x2536]`(seg_cty)、`al=[di]`、`ah=[di+1]`,`[0xd77]=ah`(`[di]` bit7 另設 `[0x4f46]&2`)。
> 即**每個 CTY section 自帶「安全(城鎮,無遭遇)vs 非安全(洞窟/迷宮,有遭遇)」旗標**——
> 正是 docs/04/11 未定名的 section header 欄位之一。remake 修正點:`game` 模式目前只在地表
> 觸發遭遇;應**載 CTY section 時讀此旗標**,非安全(迷宮/洞窟)CTY 同樣推進遭遇計數。

- 地表移動 **或** 非安全 CTY 場景(`[0xd77]==0`)時,每走一步把遭遇計數 `[0x52f4]` 減 1。
