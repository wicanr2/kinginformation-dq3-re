...
## 戰鬥背景:packbg page 0x13

`battle_enter_screen`(sub_bfd1)設文字游標後 `bp=0x13` → `lcall 1053:240` → 經
`sub_c688` → `load_packbg_page`(docs/09,page 大小 0x3d80,planar 解交錯)。即
**戰鬥畫面的藍天草原背景 = packbg.scr 第 0x13 頁**;`battle_leave_screen`(sub_c03f)
重載地圖 BLK(`load_blk`)回到地表 / 城鎮。
...
|---|---|
| 藍天草原背景 | packbg.scr page 0x13(`battle_enter_screen` sub_bfd1)|
