...
> **遭遇區範圍(機制澄清)**:遭遇戰發生在地表，以及 CTY 中由 section header
> `+0x11` 開啟遭遇的場景；不是所有 CTY 都安全，也不是只看「在不在地表」。
> IDA Pro 9.4 的 `sub_1BD97` 先比較 `4F2D` 是否為 CTY，再執行
> `cmp byte ptr ds:0D77h,0`、相等即返回，因此 **`[0xd77]==0` 是安全、非零才推進
> 遭遇計數**。這個極性也與 CTY00 sec0 的原始 header 及實機城鎮無遇敵相符。
>
> **`[0xd77]` 來源 = CTY section 旗標(RE 解出,file 0x31b2)**:載 CTY section 時
> `ds=[0x2536]`(seg_cty)、`al=[di]`、`ah=[di+1]`,`[0xd77]=ah`(`[di]` bit7 另設 `[0x4f46]&2`)。
> 即**每個 CTY section 自帶「安全 vs 遭遇」旗標**——正是 docs/04/11 未定名的 section
> header 欄位之一。remake 必須載入 raw 值並採 `!=0` gate；不可用城鎮／洞窟名稱猜測。

- 地表移動 **或** 遭遇 CTY 場景(`[0xd77]!=0`)時,每走一步把遭遇計數 `[0x52f4]` 減 1。
- 計數歸 0 → 擲新的遭遇步數,並進入 `battle_main`:
  - **事件強制遭遇**(`[0x4f46] & 0x1000`):步數 = `rng(5)+1`(短距離,連戰用)。
  - **一般遭遇**:重擲 `rng(0x12)` 直到 ≥10；IDA／`docs/60` 已證實 `[0x526c]` 是日夜
    選擇器，raw=0（非白天相位）時 `-2`，raw=1（白天）不減。
