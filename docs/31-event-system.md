...
> remake 教訓(**已解,2026-06**):早期 `game` 模式誤用 `dq3x_event`(戰鬥事件表)當對話閘;
> 現已正解並改正 —— NPC 對話走 **byte4 → section header `+0x17` bank → `dq3_dialogue_open`**(docs/42),
> `grep dq3x_event src/main.c` 零命中。對話引擎(dq3_dialogue)本身一直正確。
...
- 對話**引擎**可用(`dq3_dialogue`:真繁中劇本、分頁、控制碼)。
- **事件派發已 faithful 落地**(2026-06):NPC 對話 = byte4 → section header `+0x17` 對話 bank
  → `dq3_dialogue_open`(docs/42 全文 + 驗證);不再用 `dq3x_event` 當閘。
