...
水滴狀怪物的輪廓與 game3.png 目視吻合;其數值(HP 6 / 經驗 4 / 金錢 2)、怪名(D3TXT00
記錄 0x25d = 史萊姆)逐項對上。
...
- sprite 5(史萊姆)= 藍色水滴 + 兩眼 + 嘴,與 game3.png 的 6 隻史萊姆目視吻合；
  本節未做逐像素同狀態差分。
...
| `+0x04` | u16 | **戰鬥 MP**（→ 敵 struct `+0x10`）| `0xab63` `[bx+0xd7c]`；巴魯朋特吸 MP consumer file `0xe88a` | rec318 顯示被奪 MP，並加到施法者 `[member+0x18]` |
...
- **D3MNS 欄位 f04 / f08 / f09 / f0b / f1f / f27 精確語意**：`+0x25` 已由勝利結算
  consumer 證實為掉落閾值，`+0x26` 為掉落道具；`+0x27` 仍未知。其餘已定位 EXE 讀取點與
