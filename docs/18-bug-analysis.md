...
說清楚為什麼會出現那個現象,並提出修正。其中 3 個(巴拉摩斯打不死、彩虹水滴卡關、
五頭龍/歐里狄加當機)是會直接卡關的重大 bug,已做成同長度 binary patch、可直接套到原版 EXE
產出修正版,在 DOSBox 比對驗證。
...
| 4 | 勇者 MaxMP 成長固定 +1 | 樂趣 | 成長公式 sub_d9cc file 0xed3c | C 層(疑 dragon0.dat 表值) | 中 |
| 5 | 高等級升級錯亂 | 樂趣 | 門檻查表 sub_ecdb file 0xecdb | C 層(越界,需 clamp) | 高(根因) |
| 6 | 數值 255 溢位 | 樂趣 | 成長公式 file 0xed3c | C 層(型別/clamp) | 中 |
| 7a | 隼劍只打一次 | 樂趣 | 攻擊碼/裝備特效 sub_c8c6 | C 層(功能未實作) | 高(根因) |
