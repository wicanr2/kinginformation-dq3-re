...
- **關鍵負面發現:青衫官方 patch 與存檔修改法都沒有任何一段在修 bug#4(勇者MP+1)、bug#5(高等級升級錯亂)、
  ...
  （2026-07-28 更正：後來的 `0x1a4a8/9` patch 實際改到 VIT，已撤銷；當年與目前都沒有
  經完整實機升級驗證的 #4 patch。）
...
- **bug#4 勇者 MaxMP 成長+1**:無官方 patch。成長表在 EXE DGROUP
  (DS:0x4366→`file 0x1a4a6`,非 dragon0.dat)，真 MP 是 pair4 `0x1a4ae/f=08/02`。
  舊 `0x1a4a8/9` patch 改到 VIT，已撤銷；忠實模式保留原表。
