...
**實例**:`dq3_member_init_rng`/`gain_exp_rng`(RE sub_d9cc:`+= rng(0..(target−當前))`),既有 #4/#5/#6 單測零修改全綠(零回歸;測試總數隨後續持續成長,以實跑為準,別釘死)。
...
## 11. 別假設「單一檔含全部」:同類資產先列舉所有檔再做 pipeline

**踩坑**:精訊 DQ3 的音樂以為全在 `MBG.MCX`(18 軌),整條 pipeline(extract / SB 載入 / MT-32 render → OGG)
只處理它。**戰鬥音樂其實在獨立檔 `EBG.MCX`(6 軌)** —— 被整條 pipeline 忽略,戰鬥曲等於用錯來源(誤指 MBG 軌),
卻一直以為「音訊系統 ✅ 做完了」。是使用者一句「戰鬥好像有專屬音樂」才戳出來(再次印證教訓 2:使用者是 ground truth)。
**做法**:做某類資產(音樂/圖/字庫/表)的 pipeline 前,先 `strings EXE | grep -i '<副檔名>'` 把**所有**同類檔列全
(`mbg.mcx` **與** `ebg.mcx` 都在 EXE 字串裡,一開始就看得到),別只認第一個找到的。漏一個檔 = 子系統用錯來源還報「完成」。
記憶:`re-dq3-two-music-files-ebg-battle`。

## 12. 使用者喊「再檢查避免誤判」時,dated marker 真的會漂

**實證**:這次重查 worklist,抓到 **3 個「已完成卻標未完成」**(docker images 三件其實都在、MT-32 OGG 其實已產生、
晝夜精校的步數計數器其實這輪已 RE 出)。完成工作後沒回頭改 marker → 下一輪當未完成。對策同教訓 1/7:
完成即標 `[x]`、合併分散 worklist 成單一真相、`[ ]` 一律回 code 核實。
