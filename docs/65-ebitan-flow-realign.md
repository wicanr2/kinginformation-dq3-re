...
| **C-2 ✅(部分)** | 創角完成→sec4 `(5,5)`；開場 rec82→83→81；母親 runner 自動帶到 sec0 `(8,38)`→rec80；王座 handler56 給 50G + item `00,01,01,03,1f,1f` 並完成 `msStart`。以上由 EXE file `0x13a0..0x1633` + 本機影片定錨。TODO:能力值確認畫面、母親逐格移動動畫、完整 input trace | 核心資料已對齊 |
