...
依各段內的中斷/埠 I/O 特徵定角色(`tools/re_census.py` 統計 + 逐段掃描):

| segment | 大小 | 呼叫次數 | 角色(依特徵)|
|---|---|---|---|
| `0x111b` | ~2.7KB | 585 | **VGA planar 繪圖驅動**(out 0x3c4 Map Mask,含文字繪製器 0264)|
| `0x1053` | ~1.2KB | 183 | **檔案 I/O 庫**(int 21h ×15)|
| `0x10b6` | ~1.2KB | 122 | **滑鼠 + 雜項**(int 33h + block copy)|
| `0x109c` | ~0.4KB | 59 | **BIOS 視訊/模式設定**(int 10h)|
| `0x1104` | ~0.4KB | 50 | **鍵盤/中斷**(8042 port 0x60/0x64、PIC 0x20、speaker 0x61)|
| `0x0fe1` | ~0.6KB | 14 | DOS/檔案 + block copy(port 0x61)|
| `0x1209` | ~2.4KB | 4 | **視訊/圖形**(int 10h + 埠 + block copy)|
| `0x11dd` | ~0.7KB | 4 | 滑鼠 + 檔案(int 33h + int 21h)|
| `0x11c4` | ~0.2KB | 4 | DOS/檔案(int 21h ×6)|
| `0x100a` | ~0.6KB | 2 | DOS/檔案 |
| `0x11d3` | ~0.2KB | 1 | DOS/檔案 + 埠 |
| `0x1032` | ~0.5KB | 1 | 硬體埠 I/O |
| `0x129c` | ~5.8KB | 1 | **Sound Blaster 音效驅動**(DMA 0x0a–0x0c、PIT 0x40、PIC 0x20/21)= SBCM.LIB/Creative |

中斷整體:`int 21h`(DOS/檔案,83×)、`int 33h`(**滑鼠**,13×)、`int 10h`(BIOS 視訊)。
→ runtime = 一套自寫組語驅動:VGA planar 繪圖、Sound Blaster 音效、鍵盤、滑鼠、檔案 I/O、BIOS 視訊。
