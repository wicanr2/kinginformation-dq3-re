...
> ⚠ **2026-06-27 勘誤(待碼修)**:領悟之書真碼 = **0x4a**(非 0x40;0x40 實為勇者之盾)。
> 本 gate 在 `main.c:2321/2329` 仍寫 **0x40** → **認錯道具**(玩家拿到真領悟之書 0x4a 無法轉賢者)。
> game_tester「自洽通過」因 give/check 同用 0x40 掩蓋之。**修法**:gate 改 0x4a + 領悟之書取得改 CTY18(加爾那之塔)。
> 此修正在 audit `walkthrough-flow-audit.md` A′ 項追蹤,disasm 確認原版 gate `cmp` 碼後執行。
