...
- **給予端**:byte4=35 handler(L0x593a,give 0x66,rec38「把這個寶珠拿去吧」/rec39,flag 0x3e)
  **無 sub2 NPC 引用** → 是 runner/region 事件,原版閘在「**夜晚進村 + 開牢門**」(黑暗神燈變夜)。
  ...
  ⇒ remake 已有步數晝夜系統(2026-06-27)→ 提頓村事件目前仍用簡化:talk 提頓村 (16,2) 犯人 → 給綠寶珠 0x66 + rec29 留書(main.c 特例)。
...
> **2026-07-28 更正（以下保留為歷史誤判紀錄）**：本機原版影片與完整 handler
> `0x7425..0x7599` 證實祠堂是 **CTY70 sec0**，不是 CTY82；祭壇以 **CLEAR**
> `0x8f..0x94` 表示完成，故用 SET opcode 計數推論「build 不完整」無效。原版流程完整可觸發，
> 詳見 `docs/75-phoenix-orbs-re.md`。
...
- **道具 id 續定錨** ⚠ **(2026-06-27 勘誤:本段原 font-render 標籤在 0x40 與 0x46–0x4a 範圍位移,
  已從 D3TXT00 名表 offset+1 精確重解,對 ~9 個 gameplay 實證碼驗證)**。正確值:
  **0x40 勇者之盾、0x46 蜘蛛絲、0x47 毒蛾粉、0x48 祈禱戒指、0x49 流星手環、0x4a 領悟之書、0x4b 水槍**、
  0x4c 世界樹之葉、0x4d 生命之石、0x4e 幸運的鞋子、0x4f 生命的果實、0x50-0x54 力量/速度/耐力/聰明/幸運的種子。
  (另:0x0a 雷神之杖、0x19 魔神之斧。)**原誤標 0x40=領悟之書 已導致賢者 gate 比對錯碼,見下方勘誤。**
...
| 道具 | id | CTY/位置 | 章 | 狀態 |
|---|---|---|---|---|
| 領悟之書 | **0x4a** | **CTY18**(加爾那之塔)| Ch21(賢者轉職)| ✅ 寶箱(★碼勘誤:原寫 0x40@CTY87 誤)|
| 勇者之盾 | 0x40 | CTY87(洛特洞窟,黑暗世界)| Ch48 | ✅ 寶箱(原被誤當領悟之書/加爾那)|
...
特殊用途(後續):領悟之書→賢者轉職(達瑪)、黑暗之燈→變夜(remake 已有步數晝夜系統,黑暗之燈已接變夜)、隱身草 0x5d=朗錫爾店買非寶箱。

## Step 32:B-7 領悟之書 → 賢者轉職閘(杜勝利 Ch21)2026-06-26

領悟之書接上賢者轉職:dhama_modal 加 inv 參數 + 賢者(class 5)閘——轉賢者需持《領悟之書》或為遊人(免書);
成功消耗領悟之書(原版語意)。reclass debug 為直接換職不經此 UI 閘。

> ✅ **2026-06-27 已修**:領悟之書真碼 = **0x4a**;gate 改 `dq3_inv_find/remove(inv, 0x4a)`(原 0x40=勇者之盾,認錯道具)。
> 攻略反證(無需 disasm,gate 為 remake 新增之 Ch21 要求):CTY18=加爾那之塔(0x4a 領悟之書+力量種子+鐵頭盔)、
> CTY87=洛特洞窟(0x40 勇者之盾+生命果實)。game_tester 改測 CTY18 0x4a + CTY87 0x40(81/81);直接驗持 0x4a 轉賢者成功、無書被拒。
